...
A Basic automation account with a system-assigned identity.
...
```hcl
# Minimal call: a Basic automation account with a system-assigned identity. Applied then
# destroyed in one CI run.
locals {
  location = lookup(var.regions, var.loc, "uksouth")
  rg_name  = "rg-${var.short}-${var.loc}-${terraform.workspace}-001"
  aa_name  = "aa-${var.short}-${var.loc}-${terraform.workspace}-001"
}
